	...
	class YITH_WCAN_Session_Data_Store {
		...
		public function read( &$session ) {
			...
			if ( ! $session_data ) {
				...
				$session_data = $wpdb->get_row( $query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		...
		public function delete( &$session ) {
			...
			// delete session.
			$res = $wpdb->delete( $wpdb->filter_sessions, array( 'ID' => $id ) );
		...
		public function delete_all() {
			...
			return $wpdb->query( "TRUNCATE TABLE {$wpdb->filter_sessions}" );
		}
		...
		public function get_equivalent_session( &$session ) {
			...
			$res = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->filter_sessions} WHERE hash = %s", $hash ) );
		...
		public function get_token_by_hash( $hash ) {
			...
			$token = $wpdb->get_var( $wpdb->prepare( "SELECT token FROM {$wpdb->filter_sessions} WHERE hash = %s", $hash ) );
		...
		public function delete_expired() {
			...
			$wpdb->query( "DELETE FROM {$wpdb->filter_sessions} WHERE expiration < NOW()" );
		}
