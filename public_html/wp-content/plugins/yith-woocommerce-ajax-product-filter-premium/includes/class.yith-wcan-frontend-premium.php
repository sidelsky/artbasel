...
if ( ! class_exists( 'YITH_WCAN_Frontend_Premium' ) ) {
	...
	class YITH_WCAN_Frontend_Premium extends YITH_WCAN_Frontend {
		...
		protected function _get_shortcodes_localize( $context = 'view' ) {
			$params = array_merge(
				parent::_get_shortcodes_localize( 'edit' ),
				array(
		...
		public function filters_title( $preset = false ) {
			...
			if ( empty( $title ) ) {
				$title = apply_filters( 'yith_wcan_default_modal_title', _x( 'Filter artworks', '[FRONTEND] Default modal title - mobile only', 'yith-woocommerce-ajax-navigation' ) );
